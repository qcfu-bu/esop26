...
\draw[-latex,transform canvas={yshift=+0.5ex}] (c0.east) -- (t0.west) node [midway, above] {\small{data}};
\draw[latex-,transform canvas={yshift=-0.5ex}] (c0.east) -- (t0.west) node [midway, below] {\small{result}};
\end{tikzpicture}
\vspace{-0.8em}
...
the \Fork{} construct of the concurrency monad. In particular, when given a channel type
...
at type $\CH{P}$. The duality of the channel types allows the caller and the worker to communicate.
...
more complex concurrent algorithms. Moreover, dependent session types allow us to easily 
...
\begin{align*}
  ...
  &\quad\Return\ (\Rewrite[\textsf{reduceSplittingTree}\ xs]\ v)
\end{align*}
\endgroup
