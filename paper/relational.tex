...
$f : A \rightarrow B$ and a concurrent tree of type $\textsf{cTree A\ t}$ and
...
concurrent tree is transformed in the same way as a sequential tree under the
\textsf{map} function. Similarly, the type signature of \textsf{cReduce} states
that it takes a concurrent tree of type $\textsf{cTree}\ A\ t$ and returns a
...
more complex concurrent algorithms. Moreover, the strong correctness guarantees
provided by the dependent session types allows us to easily verify the correctness of
these derived algorithms. As an example, we implement a concurrent version of the
mergesort algorithm using the map-reduce interface and verify its correctness.

We defining the sequential mergesort algorithm as follows.
\begingroup
\small
\addtolength{\jot}{-0.2em}
\begin{align*}
  ...
  \\[0.5em]
  &\Def\ \textsf{msort}\ (xs : \textsf{list}\ \textsf{int}) : \textsf{list}\ \textsf{int} := \\
  &\quad\Match\ xs\ \With \\
  ...
  &\qquad\textsf{merge}\ (\textsf{msort}\ xs)\ (\textsf{msort}\ ys)
\end{align*}
\endgroup
