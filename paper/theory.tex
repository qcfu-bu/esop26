...
Recursive protocols can be formed using the $\fix{x : A}{m}$ construct which is typed as follows:
\begin{mathpar}
  \inferrule[Fixpoint]
  { \Gamma, x : A \vdash m : A \\ 
...
defined through $\fix{x : A}{m}$ are always productive, recursive usages of $x$ must be
