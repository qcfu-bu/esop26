...
The difficulty of integrating recursive protocols in classical session type systems
...
% introduce a concurrency monad $\mcC$ which encapsulates the side effects of
% concurrent computations. The concurrency monad is defined as follows:

% Here, the \textsc{$\mcC$Type} rule forms the concurrency monad type $\CM{A}$
% which lifts a type $A$ into a linear type. The next two rules, \textsc{Return}
% and \textsc{Let} extend the program typing judgment ${\Gamma ; \Delta \vdash m : A}$ of TLL
