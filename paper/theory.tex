...
\path[->]
  (l.north east) edge node {\footnotesize{provides types}} (p.north west)
  (p.south west) edge node {\footnotesize{subjects to verify}} (l.south east);
\end{scope}
\end{tikzpicture}
\end{center}
...
\end{mathpar}
In the \textsc{Explicit-Lam} rule, we can see that the bound variable $x$ is added to
