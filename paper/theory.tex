...
In this section, we give a brief summary of the Two-Level Linear dependent type theory (TLL)~\cite{fu25}. 
...
A ubiquitous example of ghost variables is type parameters in polymorphic functions.
For instance, the polymorphic identity function can be implemented as
\begin{align*}
...
construct protocols but not proofs for arbitrary propositions. To ensure that protocols
...
To reason about the communication channels that will appear at \emph{runtime}, the program level
...
\begin{mathpar}
  ...
  { \Gamma ; \Delta \vdash \\ 
    \epsilon \vdash A : \Proto \\
    \Delta \triangleright \Un }
  { c :_\Ln \HC{A} ; \Gamma ; \Delta \vdash c : \HC{A} }
\end{mathpar}
The protocol $A$ used in the channel types here must be \emph{closed}. 
...
\begin{mathpar}
  ...
  { \Theta ; \Gamma ; \Delta \vdash \Send\ m : \PiR{\Ln}{x : A}{\CM{\CH{B}}} }

  \inferrule[Explicit-Recv-CH]
  { \Theta ; \Gamma ; \Delta \vdash m : \CH{\ActR{?}{x : A}{B}}}
  { \Theta ; \Gamma ; \Delta \vdash \Recv\ m : \CM{\SigR{\Ln}{x : A}{\CH{B}}} }
...
\begin{mathpar}
  ...
  { \Theta ; \Gamma ; \Delta \vdash \Recv\ m : \CM{\SigR{\Ln}{x : A}{\HC{B}}} }
\end{mathpar}
...
the pair is the value of type $A$ that is received, and the second component is
